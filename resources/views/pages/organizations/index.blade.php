<x-layouts::app :title="__('Organizations')">
    ...
    <x-ui.page>
        <x-ui.page-header
            ...
            :description="__('Manage the operating boundaries for teams, workspaces, invitations, and project ownership.')"
        >
            <x-slot:actions>
        ...
        @if ($organizations->isNotEmpty())
            <div class="grid gap-4 lg:grid-cols-2">
                @foreach ($organizations as $organization)
                    <a href="{{ route('organizations.show', $organization) }}" class="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-zinc-300 hover:shadow-md dark:border-white/10 dark:bg-zinc-900/70 dark:hover:border-white/20" wire:navigate>
                        ...
                        <div class="mt-5 grid grid-cols-2 gap-3 text-sm">
                            <div class="rounded-lg bg-zinc-50 p-3 dark:bg-white/[0.03]">
                                ...
                                <p class="mt-1 font-semibold text-zinc-950 dark:text-white">{{ $organization->workspaces_count }}</p>
                            </div>
                            <div class="rounded-lg bg-zinc-50 p-3 dark:bg-white/[0.03]">
                                <p class="text-zinc-500 dark:text-zinc-400">Invitations</p>
                                <p class="mt-1 font-semibold text-zinc-950 dark:text-white">{{ $organization->invitations_count }}</p>
                            </div>
                        </div>
                    </a>
                @endforeach
